...
function displayNewForm(x){
  ...
  <input class="form-control d-inline-block w-25 ml-3" placeholder="rewrite new name" id="new-name"><input class="form-control ml-3 d-inline-block w-25" placeholder="rewrite new Url" id="new-url">
  <button class="btn btn-primary " onclick=" update(`+x+`) ">add</button>`
  
}

function update(e){
  debugger;
  var newMarkName = document.getElementById("new-name").value;
  var newMarkUrl = document.getElementById("new-url").value;
var replacedname=searchList[e].markName.replace(searchList[e].markUrl,newMarkName)
var replacedurl=searchList[e].markUrl.replace(searchList[e].markUrl,newMarkUrl)
searchList[e].markName =replacedname;
searchList[e].markUrl =replacedurl;
